sap.ui.define([
	...
	ValidateException, Export, ExportTypeCSV, Filter, FilterOperator,) 
	{

	var oView, oSAPuser, t, company, nombreUsuario;
	var Vor;
	return Controller.extend("AR_DP_REP_REPORTESEGUIMIENTOINHOUS.AR_DP_REP_REPORTESEGUIMIENTOINHOUS.controller.Reporte", {
		onInit: function () {
			t = this;
			oView = this.getView();
		...
		ConsultaVOR: function (valor) {
			...
			$.ajax({
				...
				success: function (dataR, textStatus, jqXHR) {
					var Vor = dataR;
					oView.setModel(new JSONModel(Vor), "pedidos_vor");
			...
			if (comentario != "" || comentario !== null) {
				...
				$.ajax({
					...
					error:async function (jqXHR, textStatus, errorThrown) {
						...
						let aFilters = [];
						aFilters.push(new Filter("ID_PEDIDO", FilterOperator.EQ, idp));
						aFilters.push(new Filter("MATERIAL", FilterOperator.EQ, idm));

						var vorPedido, vorMaterial, vorTipoPrecio, vorFechaCreacion,vorUsuarioCreacion,vorVin;
						await that._oDataHanaModel.read("/VorRepuesto", {
							filters: aFilters,
							success: function(resultadoDatosVor){
								vorPedido = resultadoDatosVor.results[0]["ID_PEDIDO"]
								vorMaterial = resultadoDatosVor.results[0]["MATERIAL"]
								vorTipoPrecio = resultadoDatosVor.results[0]["TIPO_PEDIDO"]
								vorFechaCreacion = resultadoDatosVor.results[0]["FECHA_CREACION"]
								vorUsuarioCreacion = resultadoDatosVor.results[0]["USUARIO_CREACION"]
								vorVin = resultadoDatosVor.results[0]["VIN"]

							jsongrabar = {
								"ID_PEDIDO": vorPedido,
								"MATERIAL": vorMaterial,
								"COMENTARIO": comentario,
								"TIPO_PEDIDO": vorTipoPrecio,
								"FECHA_CREACION": vorFechaCreacion,
								"USUARIO_CREACION": vorUsuarioCreacion,
								"FECHA_MODIFICACION": "\/Date(" + new Date().getTime() + ")\/",
								"USUARIO_MODIFICACION": oSAPuser,
								"VIN": vorVin
							};
		...
		generarEntradaAuditoria: function(nroPedido, idMaterial, comentario){
			...
			var appid = this.getOwnerComponent().getManifestEntry("/sap.app/id").replaceAll(".","/");
			var appModulePath = jQuery.sap.getModulePath(appid);
			let oModel = new sap.ui.model.odata.v2.ODataModel(appModulePath +'/destinations/AR_DP_REP_DEST_HANA/ODATA_masterPedido.xsodata', false);
			let identificador = {
			...
			if ((oView.byId("Fecha").getValue() === null || oView.byId("Fecha").getValue() === "") && oView.byId("NPedido").getValue() === "") {
				...
				$.ajax({
					...
					success: function (dataR, textStatus, jqXHR) {
						...
						// ///////////////////////////////////////// Parche por el problema del top600 en el llamado ODATA_masterPedido.xsodata/VorRepuesto
						let aFilters = [];
						if(datos){
							if(datos.length){
								for (let dato of datos){
									if (dato.Pedido) {
										aFilters.push(new Filter("ID_PEDIDO", FilterOperator.EQ, dato.Pedido));
									}
									else if (dato.Material) {
										aFilters.push(new Filter("MATERIAL", FilterOperator.EQ, dato.Material));
									} 
								}
							}else{
								if (datos.Pedido) {
									aFilters.push(new Filter("ID_PEDIDO", FilterOperator.EQ, datos.Pedido));
								}
								else if (datos.Material) {
									aFilters.push(new Filter("MATERIAL", FilterOperator.EQ, datos.Material));
								} 
							}
						}else{
							let desde = oView.byId("Fecha").getDateValue();
							let hasta = oView.byId("Fecha").getSecondDateValue();
							aFilters.push(new Filter("FECHA_CREACION", FilterOperator.BT, desde, hasta));
						}
						
						that._oDataHanaModel.read("/VorRepuesto", {
							filters: aFilters,
							success: function(resultadoDatosVor){
								oView.getModel("pedidos_vor").setProperty("/filteredPedidos_vor", resultadoDatosVor)
								var datosVor = oView.getModel("pedidos_vor").getProperty("/filteredPedidos_vor").results;
		...
		EnvioCorreo: function (evt) {
			...
			if (!oDialog) {
				...
				oDialog = sap.ui.xmlfragment(oView.getId(), "AR_DP_REP_REPORTESEGUIMIENTOINHOUS.AR_DP_REP_REPORTESEGUIMIENTOINHOUS.view.Correo", this);
		...
		popCarga: function () {
			...
			if (!oDialog) {
				// create dialog via fragment factory
				oDialog = sap.ui.xmlfragment(oView.getId(), "AR_DP_REP_REPORTESEGUIMIENTOINHOUS.AR_DP_REP_REPORTESEGUIMIENTOINHOUS.view.PopUp", this);
				oView.addDependent(oDialog);
			}
			oDialog.open();
		...
		popSuccesCorreo: function (obj, titulo) {
			...
			if (!oDialog) {
				// create dialog via fragment factory
				oDialog = sap.ui.xmlfragment(oView.getId(), "AR_DP_REP_REPORTESEGUIMIENTOINHOUS.AR_DP_REP_REPORTESEGUIMIENTOINHOUS.view.SuccesCorreo",
